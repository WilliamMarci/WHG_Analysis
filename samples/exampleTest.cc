  ...
  for(int count=0; count < numberOfEntries; count++)
  {
      ...
      for(int i=0, j=0, k=0; i<numJet; i++)
      {
	      ...
	      pJet[i]=(Jet*)branchJet->At(i);
              ...
              if( pJet[i]->BTag == 1 && pJet[i]->PT > 25 && fabs(pJet[i]->Eta) < 2.5 )
	      {
		  ptBJet[k]=pJet[i]->PT;
...
void exampleTest()
{
  gSystem->Load("/eos/home-y/youpeng/research/whg/MG5_aMC_v2_9_24/ExRootAnalysis/libExRootAnalysis.so");
  gSystem->Load("/eos/home-y/youpeng/research/whg/MG5_aMC_v2_9_24/Delphes/libDelphes.so");
  //THStack *stack;
  // access to 0-600 HT bin files
  TChain *chain = new TChain("Delphes");
  chain->Add("samples/bbmasstest.root");
  ExRootTreeReader *treeReader1 = new ExRootTreeReader(chain);
  ExRootResult *result1 = new ExRootResult();
  AnalyseEvents(treeReader1, "out.root");
